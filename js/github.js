/* =========================================================
   TEMERIA MEDIA FORGE V5 - GITHUB PUBLISH ENGINE
   Stable Publish + OG + Retry + Cache
========================================================= */
...
const CONFIG = {
  ...
  vercelBase: "https://temeria.vercel.app"
};
...
function getPublishedCardURL(fileName){
  return `${CONFIG.vercelBase}/${CONFIG.cardsFolder}/${fileName}`;
}

function getPublicAssetURL(path){
  return `${CONFIG.vercelBase}/${path}`;
}
...
function getExtensionFromDataUrl(dataUrl){
  ...
  if(dataUrl.startsWith("data:audio/mp3")) return "mp3";
...
async function getExistingSHA(apiURL, token){
  try{
    ...
    console.warn("SHA check error", err);
    return null;
  }
}
...
async function uploadMainImageIfNeeded(state, slug, token){
  ...
  setPublishStatus("Upload immagine...");

  const ext = getExtensionFromDataUrl(img);
  const base64 = extractBase64(img);

  if(!base64) return state;
...
async function uploadMP3IfNeeded(state, slug, token){
  const audio = state?.media?.mp3Url || "";

  if(!audio.startsWith("data:audio/")){
    return state;
  }

  setPublishStatus("Upload audio...");

  const ext = getExtensionFromDataUrl(audio);
  const base64 = extractBase64(audio);

  if(!base64) return state;

  const audioName = `${slug}.${ext}`;
  const audioPath = `${CONFIG.audioFolder}/${audioName}`;
  ...
  state.media.mp3Url = publicAudioUrl;
  state.media.audioMode = "mp3";

  return state;
}

async function uploadVideoIfNeeded(state, slug, token){
  const video = state?.media?.videoUrl || "";

  if(!video.startsWith("data:video/")){
    return state;
  }

  setPublishStatus("Upload video...");

  const ext = getExtensionFromDataUrl(video);
  const base64 = extractBase64(video);

  if(!base64) return state;

  const videoName = `${slug}.${ext}`;
  const videoPath = `${CONFIG.videoFolder}/${videoName}`;
  ...
  state.media.videoUrl = publicVideoUrl;
...
function ensureOGData(state, publicUrl){
  ...
  state.github.ogUrl = publicUrl;
  state.github.ogType = "website";
  state.github.ogTitle = state.content?.title || "Temeria";
  state.github.ogDescription =
    state.content?.phrase || "Temeria Media Forge";
}
...
async function publishCardToGitHub(){
  ...
  try{
    setPublishStatus("Preparazione publish...");
    ...
    const publishSlug = `${slug}-${uniqueId}`;
    const fileName = `${publishSlug}.html`;

    const publicUrl =
      getPublishedCardURL(fileName);

    ensureOGData(state, publicUrl);

    await uploadMainImageIfNeeded(state, publishSlug, token);
    await uploadMP3IfNeeded(state, publishSlug, token);
    await uploadVideoIfNeeded(state, publishSlug, token);

    setPublishStatus("Rendering HTML...");
    ...
    if(window.TemeriaForge?.safeCopyText){
      window.TemeriaForge.safeCopyText(publicUrl);
    }

    setPublishStatus("Publish completato.");
    ...
    setPublishStatus("Errore publish.");
