...
(function(){
  ...
  function collectState(){
    ...
    state.media.videoUrl = getVal("videourl").trim();
    ...
    state.media.mp3Url = getVal("mp3url").trim();
  ...
  function applyState(state){
    ...
    setVal("videourl", merged.media.videoUrl);
    ...
    setVal("mp3url", merged.media.mp3Url);
  ...
  function handleFileInput(inputId, targetKey, nameKey, urlInputId){
    const input = $(inputId);
    if(!input) return;
    ...
    input.addEventListener("change", function(e){
      ...
      reader.onload = function(ev){
        if(!window.TemeriaForge.state) window.TemeriaForge.state = clone(DEFAULT_STATE);
  ...
  window.addEventListener("DOMContentLoaded", function(){
    loadState();
    handleFileInput("giffile", "mainImage", "mainImageName", "gifurl");
    handleFileInput("minifile", "logo", "logoName", "minigif");
    renderHistoryList();
  });
})();